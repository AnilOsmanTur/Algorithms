...
class Graph:
    ...
    def weight_between(self, node1, node2):
        return self.adjMat[node1][node2]

    def edge_normalize(self):
        edges = []
        #print self.edges
        for i in self.vertexes:
            for j in range(i+1, self.vertex_n):
                if self.adjMat[i][j] != 0:
                    edges.append( [i , j, self.adjMat[i][j]] )
